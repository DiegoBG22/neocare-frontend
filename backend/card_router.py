...
def ensure_list_belongs_to_user(db: Session, list_id: int, user_id: int) -> Optional[ListModel]:
...
def ensure_card_belongs_to_user(db: Session, card_id: int, user_id: int) -> Optional[Card]:
    return (
        db.query(Card)
